...
|---|---|---|
| **U-1** | **Is `AGENT-SSOT.json` a governance owner, or an asset consumed by owners?** | An explicit user instruction. It concerns the user's own artifact and which file holds a governance concern. If asset: P-03's three rows invert, and the SSOT sections retire into the rule owners. If owner: `AGENTS.md` § Canonical Assets and § Routing both need edits no record currently plans, and the Hub's verification owner loses its routed condition |
...
| U-3 | Who owns output tone and format -- `AGENT-SSOT.json` or `USER-SSOT.json`? | Whether the format contract is company-neutral or Greyed-scoped. **D-08 assigned rendering to the user file on 2026-08-19, one day before the agent file took the same ground.** D-08's premise no longer describes the corpus, and nothing records that |
