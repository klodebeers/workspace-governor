...
- `L-014` | Trigger: reporting the status of work assigned to someone else | Finding: a status field in this repository records what was true when written, never what is true now -- and two plausible checks are worse than useless here: an unauthenticated `git ls-remote` fails on credentials rather than absence, and the **old** name of a renamed GitHub repository keeps resolving through a redirect, so its success proves nothing | Consequence: confirm completion against an authenticated live source before asking anyone to do it again; never let shell `&&` turn an auth failure into an existence claim | Owner: `AGENTS.md` § Evidence standard; `STATE.md` verification assignments
