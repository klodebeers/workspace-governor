...
| Gateway discovery no longer presupposes the canonical Hub. `hubState` is computed first (PRESENT / PRESENT_WITHOUT_RULES / ABSENT_PRE_CONSOLIDATION_SOURCES_FOUND / ABSENT); hub-derived sections carry `applicable` and `notApplicableReason` instead of returning empty results; pre-consolidation sources substitute as scan roots when the Hub is absent. Satisfies D-06 | `scripts/Invoke-GatewayDiscovery.ps1` |
...
2. ~~Revise the Gateway discovery tooling so it does not presuppose `.agents-hub` exists.~~ **Done** — see verified state. Still unexecuted; awaits the local Windows runtime test.
