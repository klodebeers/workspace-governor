...
| Directory reparse points are detected by attribute and never traversed, so an aliased junction cannot bypass the name-based B-2 rule. Read boundary stays physically inside the Hub tree | `scripts/lib/SafeTraversal.ps1` — `Test-IsDirectoryReparsePoint` |
