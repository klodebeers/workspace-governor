...
| Verification | The reply names the canonical Hub route. Silence, a guess, or a different file is a negative result and must be recorded as such. **Predicted result for Claude Code, as the Hub now stands: nothing loaded.** Claude Code reads only `CLAUDE.md`, the Hub has only `AGENTS.md`, so a negative is the correct answer about current state rather than an inconclusive test. Codex is expected to load `AGENTS.md` natively. Recorded in advance so the outcome is not misread. See open work 29z |
...
29z. **Wiring, sequenced last, executed on the local machine.** Project the Hub's root contract into each runtime's discovery mechanism. **Confirmed from vendor documentation 2026-08-21:** Claude Code reads only `CLAUDE.md` and `CLAUDE.local.md` and does **not** read `AGENTS.md`, so the Hub's root contract loads nothing in Claude Code today. The thinnest fix is a root `CLAUDE.md` containing a single `@AGENTS.md` import -- documented as the supported pattern, duplicating no governance. Note the placement constraint: an adapter inside `adapters/claude/` would never be discovered, because Claude Code only looks at a directory root and its parents. Codex reads `AGENTS.md` natively and needs nothing. This is the plan's Step 9 and is deliberately last; the operator wires it locally. `evidence/RUNTIME-CONVENTIONS-2026-08-20.md` addendum.
